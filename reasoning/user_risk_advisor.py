...
def build_user_risk_advice(
    *,
    ...
    if blocked_request_type in {"unsafe_continue_drinking", "unsafe_driving_check", "unsafe_toxic_threshold"}:
    ...
    if synthesized_payload and isinstance(synthesized_payload.get("simulation_summary"), Mapping):
        defaults = list(synthesized_payload["simulation_summary"].get("defaults_applied", []) or [])
        if defaults:
            assumptions.append("Some personal inputs were assumed because they were not provided.")
    if signals.get("age") is None:
    ...
    if emergency:
        ...
        if blocked_request_type in {"unsafe_continue_drinking", "unsafe_driving_check", "unsafe_toxic_threshold"}:
            plain_parts.append(continue_drinking_guidance)
